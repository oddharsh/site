...
export async function inspectLens(request: Request, env: Env, raw = new URL(request.url).searchParams.get("url")): Promise<{ status: number; payload: Inspection }> {
  ...
  try {
    ...
    return { status: 200, payload: {
      ...
      discovery: doors,
      readiness: { doors: doorCount, title: Boolean(title), description: Boolean(description), headings: headings.length, jsonLd },
    } };
...
function inspectionHtml(payload: Inspection): string {
  ...
  return `<h2>${escapeHtml(anatomy.title || payload.finalUrl)}</h2><p><a href="${escapeHtml(payload.finalUrl)}" rel="external noopener">${escapeHtml(payload.finalUrl)}</a></p><div class="lens-summary"><article><h3>Raw response</h3><dl><dt>Status</dt><dd>${escapeHtml(payload.status)}</dd><dt>Bytes parsed</dt><dd>${escapeHtml(anatomy.bytesRead)}${anatomy.truncated ? " (capped)" : ""}</dd><dt>Headings</dt><dd>${headings.length}</dd><dt>Links</dt><dd>${escapeHtml(anatomy.links)}</dd></dl></article><article><h3>Agent doors</h3><dl><dt>Doors observed</dt><dd>${escapeHtml(readiness.doors)}</dd><dt>Document title</dt><dd>${readiness.title ? "present" : "absent"}</dd><dt>Signed fetch</dt><dd>${payload.signed ? "yes" : "not configured here"}</dd><dt>Robots</dt><dd>${escapeHtml((payload.robots as JsonRecord).state)}</dd></dl></article></div><h3>Agent doors</h3><ul class="lens-doors">${doors.map(([path, value]) => `<li>${escapeHtml(path)}: ${(value as JsonRecord).present ? "present" : "absent"}</li>`).join("")}</ul>${headings.length ? `<section class="lens-outline"><h3>Document outline</h3><ol>${headings.map((heading) => `<li>h${escapeHtml(heading.level)} · ${escapeHtml(heading.text)}</li>`).join("")}</ol></section>` : ""}<h3>Readable text</h3><pre class="lens-text">${escapeHtml(anatomy.text)}</pre><p><a href="/lens/fetch?url=${encodeURIComponent(String(payload.url))}">Open this inspection as JSON</a></p>`;
}
