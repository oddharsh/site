...
// `lib/crawl.js`; the serendipity -> public/lib direction is established.
//
...
export function mcpServer({ serverInfo, capabilities, instructions }) {
  // `cache` is genuinely optional and now says so. Spreading undefined is a
  // no-op, and several callers (server/discover among them) deliberately omit it.
  // In a .js file TypeScript treated every parameter as optional, so the three
  // call sites in serendipity.js that relied on this were invisible until the
  // Worker became TypeScript.
