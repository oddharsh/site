...
test("the wire lens counts bytes on the wire, not the header preamble", async () => {
  const { summariseWire } = await import("./holding/_worker.js/lens-wire.js");
...
test("the KV-hit flag does not collide with the target's own cache count", async () => {
  const { summariseWire } = await import("./holding/_worker.js/lens-wire.js");
  const worker = readFileSync("./holding/_worker.js/lens-wire.js", "utf8");
  const pane = readFileSync("./holding/lens-wire.js", "utf8");
...
test("siteOf groups subdomains without a public-suffix list, and says where it is wrong", async () => {
  const { siteOf } = await import("./holding/_worker.js/lens-wire.js");
  ...
  const src = readFileSync("./holding/_worker.js/lens-wire.js", "utf8");
