...
`Refresh image toolchain` runs weekly and can also be dispatched manually. It checks the pinned `google/jpegli` revision against upstream, builds a candidate on the same macOS runner, regenerates the committed encoding study, and opens a reviewable PR when the encoder changes. The generated byte and visual diffs are evidence for the update; the PR still goes through normal CI and merge review.

The workflow also records the installed Homebrew image-tool versions in its run summary. Dependabot updates the GitHub Actions, npm, and Pillow ecosystems, but it does not manage Homebrew formulas or the upstream jpegli source commit.
