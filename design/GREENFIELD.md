...
- **Workers Cache (`cache: {enabled:true}`), evaluated 2026-07-06.** The config-driven tiered cache in front of the Worker is the right tool for a heavy-compute app and the wrong fit here. Two facts settle it: static assets are already free and tiered-cached by the assets layer, and enabling it bills every asset request at the standard rate "because each one now consults the cache in front of your Worker" (per the launch post; a BYPASS still counts, so response headers cannot carve assets back out, they only control storage). So it taxes the free, already-cached bulk to shave a sub-millisecond isolate skip off ~6 secondary routes, and its one real edge (two-tier hit rate) is noise at personal traffic. `caches.default` keeps assets free, stays surgical, and version-keys clean on deploy. Reopen only if per-hit CPU ever dominates at much higher traffic.
