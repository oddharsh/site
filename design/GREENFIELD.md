...
Against the pre-rewrite baseline: nav.js was 108,907 raw / 28,155 br q11 and built the desktop after load. Phase B now ships a generated desktop partial in every document and nav.js adopts it, so the remaining script owns behavior plus a legacy build fallback. The ledger still identifies the larger future cut: delete that fallback, move more controls to native HTML/CSS, and retain only the irreducible interaction loop. Do not claim the 3.3KB target until field and lab measurements show the current behavior is the constraint.
...
- **Workers Cache (`cache: {enabled:true}`), evaluated 2026-07-06 and superseded.** The unified Worker now uses the named `CachedPages` entrypoint selectively for reusable public routes, while the homepage, mutations, query-bearing requests, and per-visitor/inspection paths still bypass it. `caches.default` remains the local/per-colo fallback inside handlers. The design rule is therefore route-aware caching—not a sitewide cache and not a blanket refusal: cache only responses whose public/reusable contract is explicit, and keep the bypass list as part of the route oracle.
...
1. **The shell is leaving nav.js in phases.** The current site already carries generated desktop markup for curl, readers, and JS-off visitors; nav.js adopts it and keeps behavior plus a legacy fallback. The greenfield end state removes that fallback and replaces more scripted controls with native HTML/CSS, but it remains an evidence-gated follow-up rather than a claimed 88% win today.
...
4. **The counter leaves the document.** SHIPPED v133 (2026-07-02), DO-backed variant: the route ticks the existing Counter DO, so no KV seeding and increments stay atomic. Route + activation beacon makes every homepage GET pure, HEAD/bot/prerender-immune by construction, and puts the homepage in the prerender set its own counter used to bar it from. Per-visit HTML spend drops ~31KB to ~14KB. **Revised v138:** only the TICK needed to leave. The displayed count came back as SSR'd text off a read-only peek, which is lighter than a second request for 330B of SVG and keeps GETs pure all the same, since a peek mutates nothing. `/hit.svg` was renamed `/hit` once the SVG body was only the easter-egg path.
