...
holding/images/alt.json as {stem: alt}. The worker bakes it into each grid
...
prompt, so a caption reads identically whichever one produced it:
  ...
  already sitting in holding/i/ and posts those bytes to the Workers AI REST
...
Both routes read the SAME bytes (holding/i/<stem>.<hash8>.jpg is exactly what
