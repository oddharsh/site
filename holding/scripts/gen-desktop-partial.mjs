...
// once. Only the PARTIAL gets <use> refs: nav.js keeps its inline SECTION_ICONS
// because that same markup is also serialized into a per-route
// data:image/svg+xml favicon (setFavicon, nav.js), and an external <use> cannot
...
function spriteRef(name, svg) {
  const view = (svg.match(/viewBox="([^"]+)"/) || [, "0 0 32 32"])[1];
  const inner = svg.replace(/^<svg[^>]*>/, "").replace(/<\/svg>\s*$/, "");
  symbols.push(`<symbol id="${name}" viewBox="${view}">${inner}</symbol>`);
  // the outer <svg> keeps the original viewBox, so this is the same picture at
  // the same geometry; luna.css sizes it (.axp-pin .fav svg, .axp-trayico svg).
  return `<svg viewBox="${view}"><use href="/icons.svg#${name}"/></svg>`;
}
...
// ── holding/icons.svg — the sprite the partial's <use> refs resolve against ──
// Must be written AFTER the chunks above, since spriteRef() fills `symbols` as
// a side effect of building them. Served same-origin (CSP default-src 'self'
// covers external <use>) and content-hashed into /a/ by build.mjs, so the
// unhashed /icons.svg stays only as the fallback for stale HTML.
...
console.log(`holding/icons.svg: ${symbols.length} symbols, ${sprite.length}B`);
