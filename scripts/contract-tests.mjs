...
import { diffAroundRows, handleAroundChangesJson, readAroundChanges } from "../www/_worker.js/around.js";
...
test("DEPENDENCIES.md states the versions package.json actually pins", async () => {
  ...
  // dependabot merges. Measured twice in one day on 2026-08-14: it was found
  // claiming Wrangler 4.112.0 against a 4.120.0 pin, corrected, and was wrong
  // again within hours when #377 took wrangler to 4.120.1. A doc that names a
  // version nobody checks is a doc that will misinform the next reader.
  //
  ...
  //
  // The mapping is EXPLICIT rather than a scan for `<Name> <semver>`. A naive
  // scan over this file matches "Minify 0.144.0" and "CSS 1.33.0" out of the
  // middle of two names, and worse, it matches "0.28.1" and "0.28.2" out of the
  // paragraph explaining that esbuild is no longer a direct dependency and that
  // the two remaining copies are wrangler's and vitest's. Those are the versions
  // of packages this repo deliberately does NOT pin, so a scan would demand they
  // match a manifest entry that is correctly absent.
  const DOC_PINS = [
    { doc: "Wrangler",        pkg: "wrangler" },
    { doc: "Oxc Minify",      pkg: "oxc-minify" },
    { doc: "Lightning CSS",   pkg: "lightningcss" },
    { doc: "Oxlint",          pkg: "oxlint" },
    { doc: "oxlint-tsgolint", pkg: "oxlint-tsgolint" },
    { doc: "minify-html",     pkg: "@minify-html/node" },
    { doc: "TypeScript",      pkg: "typescript" },
  ];

  const pkgJson = JSON.parse(await readFile(new URL("package.json", ROOT), "utf8"));
  const pins = { ...pkgJson.dependencies, ...pkgJson.devDependencies };
  const full = await readFile(new URL("docs/DEPENDENCIES.md", ROOT), "utf8");

  // Scoped to the baseline list, and matching a FULL three-part version. Both
  // halves are load-bearing, and the second was found the naive way: a matcher
  // for `<Name> <digits>` reads "TypeScript 7.0 ships no stable programmatic
  // API" out of the Oxlint bullet and reports the pin as 7.0 against a 7.0.2
  // manifest. Same family as the quote-aware scanner in link-integrity.mjs and
  // the `<script` scanner in gotcha 17: prose that MENTIONS a version is not the
  // sentence that CLAIMS one.
  const baseline = full.slice(full.indexOf("## Current baseline"));
  assert.ok(baseline.length > 500, "DEPENDENCIES.md no longer has a Current baseline section");
  const doc = baseline;

  let checked = 0;
  for (const { doc: name, pkg } of DOC_PINS) {
    // Both directions. A package that leaves package.json has to leave this
    // table too, which is what stops the table itself going stale: esbuild was
    // retired on 2026-08-14 and this is the assertion that would have said so.
    const pinned = pins[pkg];
    assert.ok(pinned, `DEPENDENCIES.md documents ${pkg}, but package.json no longer pins it`);

    // And the doc has to still make the claim. Deleting the sentence would
    // otherwise silently drop this package from the check.
    const found = doc.match(new RegExp(`\\b${name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")} (\\d+\\.\\d+\\.\\d[\\w.-]*)`));
    assert.ok(found, `DEPENDENCIES.md no longer states a version for ${name}`);

    // Exact pins only, so a caret range in package.json is a deliberate
    // exclusion from this table rather than something to strip and compare.
    assert.match(pinned, /^\d/, `${pkg} is range-pinned (${pinned}); drop it from DOC_PINS`);
    assert.equal(found[1], pinned,
      `DEPENDENCIES.md says ${name} ${found[1]}, package.json pins ${pinned}`);
    checked++;
  }

  // Pillow lives in a different manifest and is the one non-npm pin the doc names.
  const reqs = await readFile(new URL("www/scripts/requirements.txt", ROOT), "utf8");
  const pillow = reqs.match(/^Pillow==(\S+)$/m);
  assert.ok(pillow, "requirements.txt no longer pins Pillow exactly");
  const pillowDoc = doc.match(/\bPillow (\d+\.\d+\.\d[\w.-]*)/);
  assert.ok(pillowDoc, "DEPENDENCIES.md no longer states a Pillow version");
  assert.equal(pillowDoc[1], pillow[1],
    `DEPENDENCIES.md says Pillow ${pillowDoc[1]}, requirements.txt pins ${pillow[1]}`);
  checked++;

  // Counted, for the reason the test above this one gives: a matcher that stops
  // matching asserts nothing and still reports a pass.
  assert.ok(checked >= 8, `expected the documented pin set, checked ${checked}`);
});
