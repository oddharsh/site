...
//
// D1 is the source of truth for the deploy log; www/_worker.js/checkpoints.json
...
const FILE = path.join(ROOT, "www/_worker.js/checkpoints.json");
...
let live;
try {
  const { stdout } = await run("bun", ["x", "--no-install",
...
if (SYNC) {
  ...
  console.log(`checkpoints: synced ${live.length} rows from D1 -> www/_worker.js/checkpoints.json`);
...
catch { fail("www/_worker.js/checkpoints.json is missing or unparseable — run: pnpm run checkpoints:sync"); }
...
if (diffs.length) {
  fail(
    ...
    `\n  fix with: pnpm run checkpoints:sync && git add www/_worker.js/checkpoints.json`,
  );
}
