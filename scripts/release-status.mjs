...
if (dirty) {
  ...
  console.log(`  ${bold("next")}  npm run deploy:promote          ${dim("ramp the newest version 10 -> 50 -> 100")}`);
} else if (newestLive && newestLive.pct < 100) {
  console.log(`  ${bold("next")}  npm run deploy:promote          ${dim(`finish the ramp from ${newestLive.pct}%`)}`);
} else if (staged.length) {
  console.log(`  ${bold("next")}  npm run deploy:promote          ${dim("at 100% already; this records the staged changelog rows in D1")}`);
} else {
