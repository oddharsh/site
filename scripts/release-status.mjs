...
const runJson = async (args) => {
  const out = await run("bun", ["x", "--no-install", "wrangler", ...args]);
...
let staged = [];
try {
  const committed = JSON.parse(await readFile(new URL("../www/_worker.js/checkpoints.json", import.meta.url), "utf8"));
