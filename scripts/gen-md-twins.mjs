...
//
//   node scripts/gen-md-twins.mjs [outDir]     # default: .build/holding
//
...
const SITEMAP = "holding/sitemap.xml";
const HAND_DIR = "holding/md";
...
export function htmlFileFor(surface, root = ".") {
  ...
  const candidates = p === "/"
    ? ["holding/index.html"]
    : [`holding${p}.html`, `holding${p}/index.html`];
...
export function buildTwins(root = ".", opts = {}) {
  ...
  for (const surface of manifest.surfaces) {
    ...
    // holding/index.html would; it also already ships as a static asset and
    // already answers `Accept: text/markdown`. Generating over it would be a
    // regression dressed up as consistency.
    if (existsSync(join(root, "holding", twinPath(surface.path).replace(/^\//, "")))) {
    ...
    const generatedAbs = opts.generatedRoot
      ? join(opts.generatedRoot, "holding", `${surface.path === "/" ? "index" : surface.path.slice(1)}.html`)
      : null;
...
export const TWIN_FACTS = [
  {
    twin: "holding/md/bot.md",
    facts: [
      {
        label: "User-Agent",
        source: "holding/_worker.js/lib/botauth.js",
      ...
      { label: "JWKS path", source: "holding/_worker.js/bot.js", string: "/.well-known/http-message-signatures-directory" },
      { label: "sig1 algorithm", source: "holding/_worker.js/bot.js", string: "Ed25519" },
      { label: "sig2 algorithm", source: "holding/_worker.js/bot.js", string: "ML-DSA-44" },
    ],
  },
  {
    twin: "holding/md/whoareyou.md",
    facts: [
      ...
      { label: "WebMCP bridge", source: "holding/_worker.js/whoareyou.js", string: "/.webmcp/bridge.js" },
    ],
  },
  {
...
function main() {
  const outDir = process.argv[2] || ".build/holding";
