...
- **Authoring is buildless; SERVING is minified.** `build.mjs` (repo root, with exact Oxc Minify and Lightning CSS devDependency pins) copies `holding/` to `.build/` and minifies the five shell scripts (`nav.js`, `notepad.js`, `lens.js`, `lens-browser.js`, `quiz.js`) plus `luna.css` (owner-approved 2026-07), deploying each readable original alongside as `/<name>.src.js` / `/luna.src.css` (the banner in each minified file points there). Oxc targets ESNext and keeps top-level names stable; Lightning CSS performs the CSS minification and strict parse checks. Everything else — `index.html`, all garage/lwe HTML, images, `_headers`, worker modules — ships byte-identical to git. Wrangler still bundles the Worker modules with its own nested esbuild; that is separate from the site minification toolchain. Do NOT extend the build into bundling, HTML minification, version auto-bumps, or CSS beyond `luna.css`; it exists for those six files only.
