...
- **holding/** (aadhar.sh): a **Cloudflare Worker with static assets** (migrated off Pages 2026-06-30). Config is `wrangler.jsonc` at the repo root: it points `main` + `assets.directory` at `.build/holding` and runs `build.mjs` via its `build.command`, so `assets.run_worker_first` (an allowlist mirroring the `ROUTES`/`PREFIX` tables in `index.js`; static is the default) applies to the built tree; `workers_dev:false` (custom domain only). **Deploy: `wrangler deploy` or `npm run deploy` (identical, both self-build via `build.command`) from the repo root.** Because the config self-builds, any deploy path (including a `git push` via Workers Builds, if connected) ships the minified tree; there is no bare-deploy that serves the readable originals. Local dev uses `wrangler.dev.jsonc` (readable `holding/`, fast reload). Verify after every deploy with `node verify-routes.mjs https://aadhar.sh` (now also asserts `/nav.js` minified + `.src` twins resolve). Bindings live in `wrangler.jsonc` (KV/R2/D1/DO); secrets via `wrangler secret put` (`RN_SIGNING_KEY_JWK`, `BROWSER_RENDER_TOKEN`, `RN_BUST_SECRET`).
- **serendipity/** (separate Worker, aadhar.sh/serendipity): `cd serendipity && wrangler deploy`.
- **cal/** (coffee booker): **LIVE** at aadhar.sh/coffee (zone route → `cal-aadhar-sh` worker). Deploy separately: `cd cal && npm run deploy` (secrets already set; the cal scripts pass `-c wrangler.toml` so a bare `wrangler deploy` from cal/ does not inherit the repo-root `build.command`). Availability serves from an SWR calendar snapshot (KV `cal:busy`, 2s upstream deadline, stale fallback); the GET page edge-caches 30s; booking fails closed if the calendar can't be vouched for. See [cal/README.md](cal/README.md).
...
- **Deploy = `wrangler deploy` (or `npm run deploy`, identical) from the repo root**; a `git push` to main does the same via Workers Builds. The deploy config (`wrangler.jsonc`) points `main` + `assets` at `.build/holding` and runs `build.mjs` first through its `build.command`, so EVERY deploy path self-builds and ships the minified shells + `luna.css`. There is no longer a bare-`wrangler deploy` that serves the readable originals (that footgun is how prod once ran a 78KB unminified `nav.js`). Local dev is the exception: `wrangler dev -c wrangler.dev.jsonc` (`npm run dev`, wired into `.claude/launch.json`) serves the readable `holding/` tree directly so the edit→reload loop never pays the full `.build` copy. Before deploying, `npm run perf-budget` gates the deterministic budgets (luna.css parses, the three shells + luna.css are minified with banners and under their byte ceilings, worker bundle under 70KB gzip). After deploying, run `node verify-routes.mjs https://aadhar.sh` — it now also asserts `/nav.js` is minified (banner + <50KB) and every `.src` twin resolves.
...
- **Authoring is buildless; SERVING is minified.** `build.mjs` (repo root, one devDependency: esbuild) copies `holding/` to `.build/` and minifies the three shell scripts (`nav.js`, `notepad.js`, `lens.js`) plus `luna.css` (owner-approved 2026-07), deploying each readable original alongside as `/<name>.src.js` / `/luna.src.css` (the banner in each minified file points there). It also hard-fails the deploy if `luna.css` doesn't parse as valid CSS (the v143 corruption slipped through for three releases because esbuild only warns). Everything else — `index.html`, all garage/lwe HTML, images, `_headers`, worker modules — ships byte-identical to git. Do NOT extend the build into bundling, HTML minification, version auto-bumps, or CSS beyond `luna.css`; it exists for those four files only.
