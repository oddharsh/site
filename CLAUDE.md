...
17. **`script-src` is per-document sha256 hashes, and the committed map is EMPTY
    ...
    `/lens?url=` framed view (ditto). `/security` (2026-09-16), `/whoareyou`,
    `/garage/dyno`, `/ledger` and `/around` (all 2026-09-25) are built documents with their live values fetched after load;
