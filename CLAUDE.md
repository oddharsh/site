...
| `src/content/md/` | Hand-authored Markdown twins for the Worker-rendered prose pages, whose text lives in template literals no build step can read. SEVEN of them: `/around`, `/bot`, `/coffee`, `/lens`, `/security`, `/whoareyou`, `/garage/dyno`. It was three when this row was written; `/terminal` joined in #227 (and left with the page on 2026-09-16), the next three in #354, and `/garage/dyno` on 2026-09-02, and nothing here noticed the first two times, because the set is not a list anywhere in the code. `buildTwins` looks for `<path>.md` in this directory per SURFACE, so any page gets a hand twin by dropping a file in, and a hand twin OUTRANKS the generated tier. `.assetsignore`d (build input, not a public URL) and staged into the served tree: the generator publishes each at its page's own path, `/bot.md` and so on. `checkTwinFacts()` pins FOUR of the seven (`bot`, `whoareyou`, `security`, `garage-dyno`) against the Worker in BOTH directions, so bumping `BOT_VERSION` fails the deploy until `bot.md` agrees; the other three carry no pins and can drift from the page silently. `security.md`'s pins read `lib/security.ts` rather than the page, since a page ABOUT headers must agree with the module that SENDS them; one of them is derived from the header set `cspHeadersFor` actually emits, so reintroducing a report-only twin fails the deploy until `security.md` stops claiming `'unsafe-inline'` is gone. It read the `ENFORCE_PAGE_HASHES` flag until that flag was deleted on 2026-08-23; reading the emitted headers is the better check, since a flag states an intention and the headers state what ships. |
...
[add-photos.sh] — resize, rotate, encode:
   |   1. sips -s format tiff, HEIF/HIF ONLY, at FULL resolution and 16 bits.
   |      A JPEG source skips it: zenc decodes those itself.
   |   2. zenc square --orient N --transfer <srgb|g22>, ONE invocation, which
   |      decodes once and writes all three tiers from the same linear-light
   |      frame. --orient is an exact sample permutation (it was jpegtran
   |      -rotate N until 2026-08-26, silently lossy off iMCU alignment,
...
`add-photos.sh` recorded dropping the `sips -Z 2000` pre-reduction as MEASURED
AND DECLINED: over 6 sources, mean linear luminance moved 0.02286 to 0.02270,
inconsistent in direction, for +139% pipeline time. Re-measured with
ssimulacra2 it is +27.5 mean over 10 frames and better on 10 of 10.

Both numbers are correct about what they measured, which is the whole lesson.
**Mean linear luminance is one scalar per frame and is blind to everything the
defect actually was**: a gamma-incorrect first reduction misplaces energy
WITHIN the frame while preserving its average almost exactly, and the metric
was chosen precisely because it needed no reference. That property is what made
it safe from reference bias and also what made it unable to see this.

The reversal was re-verified independently on 2026-08-26 before it was
...
the same shape from other directions). Before trusting a null result, ask what
...
### `/ask` — NLWeb, and the two things a compliance checklist cannot see

NLWeb is a thin convention: answer natural language at `/ask`, return schema.org
objects, expose the same thing over MCP. This origin had the MCP half and a
search index over its own corpus, so `src/worker/nlweb.ts` is the endpoint, the
parameter contract, and a schema.org projection of the search records. The `ask`
MCP tool calls the same function, because the protocol specifies both doors
carrying the same arguments and the same result, and two rankings under one name
is exactly the drift `lib/tools.ts` exists to prevent.

**STREAMING DEFAULTS TO TRUE**, which reads as a bug the first time you curl it.
A bare `GET /ask?query=x` is an event stream; `streaming=0` (or `false`) returns
one JSON body. That is the spec's own default and the route oracle asserts BOTH,
because a regression flipping it leaves every other assertion passing.

**The streaming DIALECT is selected by the request shape and by nothing else.**
The reference server checks whether `query` arrived as an OBJECT: if it did, the
request is v0.55 and the answer is NAMED SSE events (`event: start` / `result` /
`complete`); otherwise it is legacy and the frames are unnamed `data:` lines
carrying `message_type`. No header, no query parameter, and no `Accept` value
selects it. Worth knowing before anyone tries to negotiate it the usual way,
since a server emitting named events at a legacy caller is talking to nobody.

**Two modes are REFUSED rather than degraded.** `mode` is `list`, `summarize` or
`generate`, and the last two need a language model. This Worker has no AI
binding, so both answer 501 naming `supported_modes` instead of quietly serving
`list`. `prev` gets the same treatment: decontextualizing a follow-up is a
model's job, so a caller who sends it without `decontextualized_query` gets their
raw query searched AND a `_meta.decontextualization` line saying so. Descriptions
are extractive, and `_meta.description` says `extractive` because the spec's own
field annotation reads "(generated by an llm)" and ours are not.

That is the part a checklist cannot see, and it is the reason the honest answer
to "are we NLWeb compliant" is longer than a yes. Every one of those three could
have been a 200 that looked right.

`score` is 0-100 and is normalised against **what the query could have scored**
(terms x 13, the per-term ceiling in `search.ts`), never against the response's
own top hit. A relative normalisation hands every query a 100 and makes the
number comparable only inside one answer.

`schema_object` is projected from the search record's `kind`: `page` to WebPage,
`writing` to BlogPosting, `document` to DigitalDocument, `utility` to WebAPI, and
each one is joined by `@id` to the WebSite node the homepage's JSON-LD already
...
door the doors tier only knocks on.**
`lensProbeNlweb` reads a status code from a request carrying no query, which is
deliberately conservative and is also how four of six origins in a 46-site survey
reached Agent-Native on a `/ask` that answered 410, 412, 429 or 401. The lens
sends one real question and grades the answer against the result contract PER
FIELD, because partial conformance is the normal case and "8 of 10 results carry
a `schema_object`" is the sentence an agent author actually needs. It reports the
framing and dialect too, since which one an origin answers in is a real fact
about how current its implementation is.

It is opt-in, cached 1h per origin AND query, and rate-limited to 4/min
(`LENS_RL_NLWEB`), which is tighter than the catalogue read beside it. The reason
is cost asymmetry: `tools/list` asks a server to describe itself, and `/ask` asks
a retrieval endpoint a question that on a real instance is a vector search and in
`generate` mode is somebody's model call. So the request pins `mode=list`
EXPLICITLY rather than trusting their default, and a contract test asserts that
exactly three parameters reach the target and that only `url` and `q` are read
from the caller.

The cache key includes the QUESTION. Keying on origin alone would show one
...
Three rules decide the planner in `src/client/lens-tools.js`, and the first is the one
