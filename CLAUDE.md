...
| `scripts/gen-feeds.mjs` | Generates the committed RSS 2.0 projections at `/writing/feed.xml`, `/garage/feed.xml`, and `/lwe/feed.xml` from `posts.json`, the writing text, `site-manifest.json`, and the sitemap dates. `build.mjs` regenerates them in memory and fails on drift, so local dev and production serve the same static bytes without adding the renderer or source-file fan-out to the Worker runtime. |
