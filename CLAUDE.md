...
| `holding/infotip.js` | The same trick for the SHELL: Explorer-style infotips on taskbar app buttons, tray icons, the clock, desktop icons, title-bar controls, and any form field carrying a tip. Every one of those already had a `title=`, so this is what the OS tooltip was hiding — a pin's page count (from nav.js's own destination table), the tray's live colo/build (the same JSON its click-balloon fetches), the clock's full date. It re-draws `title` in Luna rather than replacing it: the attribute is emptied on hover and restored on leave, so AT still reads it at focus and a page with no JS keeps the native tooltip. `[data-tip]` is the explicit opt-in for a control that never had one. **`nav.js` owns the target selector and passes it in** (a second copy is how a loader and its module come to disagree about what has a tip), and it loads on the first chrome hover, never on a coarse pointer. Windows' two delays live in `hoist.js` now (`openMs` 400 cold-dwell, `autopopMs` 6s), both defaulting OFF so the content surfaces keep the instant show they were tuned for. |
