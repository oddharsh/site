...
| `holding/_worker.js/result-receipt.js` + `holding/.well-known/result-receipt-v1.json` | The root MCP's portable result-receipt contract. Every successful `/mcp` tool call gets a required `_receipt` binding origin, issue time, tool arguments, unreceipted result, server version, and deployed Worker version. Production signs the RFC 8785 canonical receipt (with only `proof.signature` omitted) using the existing Ed25519 `RN_SIGNING_KEY_JWK`; this binds the proof's algorithm, key ID and canonicalization too. Verifiers accept only the canonical aadhar.sh key directory, never an arbitrary substituted key URL. Local dev emits an explicit unsigned proof because `.dev.vars` is intentionally absent. Receipts are returned rather than persisted, so public reads stay reads and do not create attacker-controlled D1 growth. This is root-only: Serendipity shares the protocol decorator but does not claim a result shape it does not return. |
...
- **Portable `_receipt` on every successful root-tool result.** Its origin and
  issue time establish where and when the result was produced; RFC 8785 +
  SHA-256 digests bind the arguments and unreceipted structured result; the
  producer block carries both server and Cloudflare Worker versions; and an
  Ed25519 proof lets the receipt survive outside the original HTTPS exchange.
  "Durable" means portable and independently verifiable, NOT stored: persisting
  every public call would turn read-only tools into writes and hand an attacker
  an unbounded D1 append surface. The live `tools/list` output schemas require
  `_receipt`, and Lens observes those schemas through a bounded catalog read; it
  never invokes a foreign tool. The optional Lens check does not change the
  readiness score. The schema is site-local, not represented as an MCP standard.
...
3. **Authoring stays buildless; serving is minified, on every page.** The only
   ...
   `.build/holding`, so no deploy path can ship the readable originals. Local
   development uses `wrangler.dev.jsonc` against readable `holding/`. Never bundle
