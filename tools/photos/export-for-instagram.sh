...
finish() {  # finish <candidate.jpg> <stem> <source> — place it, metadata handled
  ...
  # Edit the temporary candidate first, preserving any existing export if the
  # metadata operation fails. Explicit returns also hold inside $(finish ...),
  # where Bash does not inherit errexit.
  if [ "$KEEP_EXIF" -eq 1 ]; then
    ...
    exif-sooc -TagsFromFile "$src" -all:all "-Orientation#=1" -overwrite_original "$cand" >/dev/null || return
  else
    exif-sooc -all= -overwrite_original "$cand" >/dev/null || return
  fi
  mkdir -p "$OUT" && cp "$cand" "$dest" && stat -f%z "$dest"
}
