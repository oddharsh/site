...
export function renderDesktopArtifacts(surfaces = readManifest().surfaces) {
  ...
  const trayHtml = String(TaskbarTray({ items: TRAY_ITEMS.map((item) => ({
    id: item.id, hidden: item.hidden, href: item.href, kind: item.kind,
    title: item.title, label: item.label,
    icon: unsafeHtml(spriteRef(`tray-${item.kind}`, item.svg)),
  })) }));
  ...
  const sprite = `<svg xmlns="http://www.w3.org/2000/svg" width="${spriteWidth}" height="${spriteHeight}" viewBox="0 0 ${spriteWidth} ${spriteHeight}">`
    + "<!-- GENERATED by gen-desktop-partial.ts from shell-data.ts. Do not hand-edit. Served from /a/ as plain brotli, never as a dcz dictionary delta; see DICTIONARY_TYPES in _worker.js/lib/assets.js. -->"
