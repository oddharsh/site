...
export function renderDesktopArtifacts(surfaces = readManifest().surfaces) {
  ...
  const sprite = compactSvg(`<svg xmlns="http://www.w3.org/2000/svg" width="${spriteWidth}" height="${spriteHeight}" viewBox="0 0 ${spriteWidth} ${spriteHeight}">`
...
// Measured 2026-09-26: 19,832 B raw to 15,819, and 2,706 B brotli to 2,527.
//
...
// Two byte-for-byte equivalences over OUR authored art, never a general SVG
// minifier. An empty element closes itself (the tray art was written with
// `<stop ...></stop>`), and three attributes that restate the spec's initial
// value go: x1/y1 on a linearGradient (0%), offset on a stop (0), and
// flood-color on a filter primitive (black). Worth 74 B brotli on the sprite
// after the hoist. feDropShadow's dx and dy stay, because their default is 2.
export const compactSvg = (svg: string) => svg
  .replace(/<(\w+)([^<>]*?)><\/\1>/g, "<$1$2/>")
  .replaceAll(' x1="0" y1="0"', "")
  .replaceAll('<stop offset="0"', "<stop")
