...
//
//   node tools/gen-manifest.ts        # regen the worker module + Run fences
//
...
import type { SiteManifest, Surface } from "./site/generated/manifest.ts";
...
export function readManifest(): SiteManifest {
...
const seg = (p: string) => p.split("/").pop();
const paletteLabel = (s: Surface) => `${s.section} · ${s.short || seg(s.path)}`;
const navEntry = (s: Surface) =>
...
export function navFenceBody(surfaces: Surface[], section: string) {
  return surfaces
...
export function agentSurfaces(surfaces: Surface[]) {
  return surfaces
...
export function webmentionPaths(surfaces: Surface[]) {
  return surfaces.filter((s) => s.flags.webmention).map((s) => s.path);
}
export function webmentionSections(surfaces: Surface[]) {
  return surfaces.filter((s) => s.flags.webmention && s.kind === "section").map((s) => s.path);
}

export function workerModule(surfaces: Surface[], projection = {
  agentSurfaces: agentSurfaces(surfaces),
  webmentionPaths: webmentionPaths(surfaces),
  webmentionSections: webmentionSections(surfaces),
}) {
...
export const AGENT_SURFACES = ${JSON.stringify(projection.agentSurfaces, null, 2)};
...
export const WEBMENTION_PATHS = ${JSON.stringify(projection.webmentionPaths, null, 2)};
export const WEBMENTION_SECTIONS = ${JSON.stringify(projection.webmentionSections, null, 2)};
`;
}
...
function main() {
  // Validate before touching either output. Cargo reuses the pinned workspace's
  // binary; ordinary site builds keep the pure projection oracle below.
  const projection = JSON.parse(execFileSync("cargo", [
    "run", "--quiet", "--locked", "--manifest-path", "tools/site/Cargo.toml",
    "--bin", "site-model", "--", "project", MANIFEST,
  ], { encoding: "utf8" }));
  const { surfaces } = readManifest();
  const worker = workerModule(surfaces, projection);
  if (worker !== workerModule(surfaces)) throw new Error("Rust manifest projection disagrees with build oracle");

  const rustOutput = (command: string) => execFileSync("cargo", [
    "run", "--quiet", "--locked", "--manifest-path", "tools/site/Cargo.toml",
    "--bin", "site-model", "--", command,
  ], { encoding: "utf8" });
  ...
  writeFileSync(WORKER_MOD, worker);
