...
if (SYNC) {
  ...
  console.log(`checkpoints: synced ${live.length} rows from D1 -> src/worker/checkpoints.json`);
...
catch { fail("src/worker/checkpoints.json is missing or unparseable — run: pnpm run checkpoints:sync"); }
...
if (diffs.length) {
  fail(
    ...
    `\n  fix with: pnpm run checkpoints:sync && git add src/worker/checkpoints.json`,
  );
}
