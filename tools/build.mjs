...
// The served tree is composed from three source directories. Several invariants
// want to read "everything that will be served", which used to be one readdir of
// public/. This keeps that one idea in one place instead of three guesses.
...
async function checkInvariants() {
  ...
  try {
    ...
    for (const dir of ["garage", "lwe"]) {
      for (const f of await readdir(`src/pages/${dir}`)) {
...
// images/meta is DERIVED (from images/exif.json + images/histograms.json) and
// gitignored since #441, so there is nothing to stage and staging it would
// re-create a second copy of data that is already committed once.
//
// main's other STAGE_SKIP entry, www/scripts, is gone rather than repointed:
// the photo pipeline left the served tree entirely for tools/photos, so it is
// not a candidate for staging in the first place.
const STAGE_SKIP = new Set(["public/images/meta"]);
// The served tree is COMPOSED from four source directories now. Its shape under
// .build/public is unchanged, so every public URL is where it was; what moved is
// where each file is authored. public/ is byte-for-byte assets, src/pages/ is
// every HTML document, src/content/ is authored prose and registries, and
// src/client + src/styles supply the shell. src/dict/ is deliberately NOT here:
// the committed dictionary snapshots are build INPUT and were only ever staged
// so .assetsignore could exclude them again.
...
const minifiedPage = (staged, rel) => {
  ...
  return banner + minifyHtml.minify(
    Buffer.from(transformInlineHtmlBlocks(staged, `src/pages/${rel}`)),
    HTML_MINIFY_CFG,
  ).toString();
};
