...
try {
  // `bun info` rather than `npm view` or `pnpm view`: this repo pins `packageManager`,
  // and asking any other client for a registry fact is one more thing to install.
  // stderr is dropped because a client can warn about workspace configuration on
  ...
  const stable = execFileSync("bun", ["info", "bun", "version"], {
    encoding: "utf8",
    stdio: ["ignore", "pipe", "ignore"],
  }).trim();
  console.log(`  newest STABLE bun on npm: ${stable}`);
} catch { /* offline; the verdict above does not depend on it */ }
