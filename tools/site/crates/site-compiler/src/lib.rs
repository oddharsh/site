...
impl Plan {
    pub fn validate(&self) -> io::Result<()> {
        ...
        for job in &self.jobs {
            ...
            if let Action::Brotli(BrotliOptions { quality, window }) = job.action {
        ...
        for output in &outputs {
            for parent in Path::new(output).ancestors().skip(1) {
                if outputs.contains(&parent.to_string_lossy().into_owned()) {
                    return Err(invalid(format!(
                        "output path conflicts with an ancestor: {output}"
                    )));
                }
            }
        }
...
fn encode(bytes: &[u8], action: Action) -> io::Result<Vec<u8>> {
    match action {
        Action::Identity(_) => Ok(bytes.to_vec()),
        Action::Brotli(BrotliOptions { quality, window }) => {
...
/// The adapter consumes these declarations; serialization and types share one definition.
pub fn typescript() -> String {
    let config = ts_rs::Config::default();
    let mut output = String::from("// Generated by site-compiler; run bun run gen:manifest.\n");
    for declaration in [
        Action::decl(&config),
        IdentityOptions::decl(&config),
        BrotliOptions::decl(&config),
        Job::decl(&config),
        Plan::decl(&config),
        Artifact::decl(&config),
    ] {
...
mod tests {
        ...
        Plan {
            ...
            jobs: ["a", "b"]
                .map(|name| Job {
                    ...
                    action: Action::Brotli(BrotliOptions {
                        quality: 4,
                        window: 24,
                    }),
                })
    ...
    fn compiler_and_action_changes_invalidate_cached_work() {
        ...
        plan.jobs[0].action = Action::Identity(IdentityOptions {});
    ...
    #[test]
    fn generated_adapter_contracts_are_current() {
        let path =
            std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../generated/compiler.ts");
        assert_eq!(
            super::typescript(),
            std::fs::read_to_string(path).unwrap(),
            "run bun run gen:manifest"
        );
    }

    #[test]
    fn unknown_transform_options_are_rejected() {
        for action in [
            serde_json::json!({"kind":"identity", "unexpected":true}),
            serde_json::json!({"kind":"brotli", "quality":11, "window":24, "unexpected":true}),
        ] {
    ...
    fn malformed_plans_fail_before_any_writes() {
        ...
        duplicate.jobs[1].output = duplicate.jobs[0].output.clone();
        assert!(f.run(&duplicate, "one").is_err());
        for (quality, window) in [(12, 24), (11, 25)] {
            let mut invalid = plan();
            invalid.jobs[0].action = Action::Brotli(BrotliOptions { quality, window });
            assert!(f.run(&invalid, "one").is_err());
        }
        let mut conflict = plan();
        conflict.jobs[0].output = "directory".into();
        conflict.jobs[1].output = "directory/file".into();
        assert!(f.run(&conflict, "one").is_err());
