...
per-photo meta files (assets/photos/data/meta/<stem>.json), at photo-add time.
...
encoded JPG twin in assets/photos/thumbs/ (content-hashed, found via hashes.json) —
...
try:
    ...
    sys.stderr.write("error: Pillow not installed. run: python3 -m pip install -r tools/media/requirements.txt\n")
    sys.exit(2)

BINS = 64
REPO = Path(__file__).resolve().parents[2]
IMAGES = REPO / "assets" / "photos" / "data"
HASHED = REPO / "assets" / "photos" / "thumbs"
META = IMAGES / "meta"
