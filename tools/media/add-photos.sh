...
#   1. generates the grid thumbnails at assets/photos/data/<stem>.{jpg,avif} +
...
#
# usage:
#   ./tools/media/add-photos.sh /path/to/photo.HIF
#   ./tools/media/add-photos.sh /path/to/folder/
#   ./tools/media/add-photos.sh /path/a.jpg /path/b.HIF /path/folder/

set -e

# resolve from anywhere — assumes script lives at tools/media/
SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
PROJECT_DIR="$( cd "$SCRIPT_DIR/../.." && pwd )"
DEST="$PROJECT_DIR/assets/photos/data"
...
# zenc (tools/media/zenc) is the JPEG encoder: a zenjpeg wrapper running
...
# content-address every tier into assets/photos/thumbs/ + refresh hashes.json (the
# worker bakes /i/ URLs from that map; idempotent, only new bytes copy)
"$SCRIPT_DIR/hash-thumbnails.sh" 2>&1 | tail -1

# ── the committed photo index (content/data/photo-index.json) ──
...
INDEX_FILE="$PROJECT_DIR/content/data/photo-index.json"
...
# it reads the committed assets/photos/thumbs/ square via hashes.json and posts those exact
...
node "$PROJECT_DIR/tools/media/check-photo-pipeline.mjs"
echo ""
