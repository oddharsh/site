...
#
# For every tier file in assets/photos/data/ (<stem>.avif, <stem>.jpg,
# <stem>-400.avif) this computes sha256, copies the bytes to
# assets/photos/thumbs/<name-with-.hash8-before-ext>, and writes
# assets/photos/data/hashes.json ({stem: {a,j,s}}), which buildImagesManifest
...
# an incremental add only stages the NEW stems in assets/photos/data/ (the earlier
# tiers were git-rm'd after they were hashed into assets/photos/thumbs/), so a from-scratch
...
SRC_DIR="$PROJECT_DIR/assets/photos/data"
OUT_DIR="$PROJECT_DIR/assets/photos/thumbs"
...
#      so assets/photos/thumbs/ is 1:1 with hashes.json and check-photo-pipeline.mjs passes.
