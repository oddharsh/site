...
#
# After running, re-run hash-thumbnails.sh: it re-hashes each tier into assets/photos/thumbs/
...
#   SQ=600 ./tools/media/reencode-thumbnails.sh
#   SQ=600 SQ_SM=400 ./tools/media/reencode-thumbnails.sh "/path/to/source/folder"
#
set -euo pipefail
...
# Enumerate published stems from the content-hashed JPG tiles in assets/photos/thumbs/. The
...
STEMS=$(for j in "$PROJECT_DIR/assets/photos/thumbs/"*.jpg; do b=$(basename "$j" .jpg); echo "${b%.*}"; done | sort -u)
TOTAL=$(echo "$STEMS" | grep -c . || true)
[ "$TOTAL" -gt 0 ] || { echo "error: no published thumbnails found in assets/photos/thumbs/ (expected the content-hashed JPG tiles)" >&2; exit 1; }
