...
// assets/photos/data/semantics.json as {stem: {terms, from}}.
//
// ── why this exists ───────────────────────────────────────────────────────
...
//
// Usage:
//   node tools/media/gen-photo-semantics.mjs              # derived only
//   node tools/media/gen-photo-semantics.mjs --vision     # + model terms
//   node tools/media/gen-photo-semantics.mjs --vision --dry-run
//
...
const DATA = path.join(ROOT, "assets/photos/data");
const THUMBS = path.join(ROOT, "assets/photos/thumbs");
const META = path.join(DATA, "metadata.json");
const HASHES = path.join(DATA, "hashes.json");
const ALT = path.join(DATA, "alt.json");
const OUT = path.join(DATA, "semantics.json");
...
async function visionTerms(stem, hashes) {
  ...
  const file = path.join(THUMBS, `${stem}.${entry.j}.jpg`);
